...
public class CompraAmarreImpl implements CompraAmarreService {
    ...
    public CompraAmarre buscarPorId(Long id) {
        ...
        return repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Compra de Amarre", id));
    }

    @Override
    public void eliminar(Long id) {

        CompraAmarre compra = repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Compra de Amarre", id));
